...
@export var _enemy_to_spawn: PackedScene = null
...
func _trigger_spawn() -> void:
	...
	if(GameManager.current_level_scene == null):
		return
	elif (currentLevel._max_enemies > currentLevel._current_enemies.size()):
		var new_enemy: EnemyCharacter = _enemy_to_spawn.instantiate() as EnemyCharacter
